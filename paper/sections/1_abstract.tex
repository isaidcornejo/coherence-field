...
\mathcal{A}(\theta;q)
=
G^{ij}(\theta)\,\big(C_{ij}(\theta;q)-G_{ij}(\theta)\big),
\]
...
To account for finite-sample effects inherent to empirical estimation, we
further introduce a resolution-aware refinement that distinguishes formal
geometric alignment from resolvable coherent alignment, without modifying the
underlying invariant construction.
